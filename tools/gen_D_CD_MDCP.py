...
def call_matrix_api(
    ...
    mdcp_count = len(mobile_distribution_centers)

    sources = list(range(cd_count))
    destinations = list(range(cd_count, cd_count + mdcp_count))
    ...
    distance_matrix = pd.DataFrame(
        ...
        columns=mobile_distribution_centers["mdcp_name"].tolist(),
    )
...
def compute_all_route_geometries(
    ...
    for _, cd_row in distribution_centers.iterrows():
        ...
        for _, mdcp_row in mobile_distribution_centers.iterrows():
            mdcp_name = mdcp_row["mdcp_name"]
            destination_lat = float(mdcp_row["latitude"])
            destination_lon = float(mdcp_row["longitude"])

            print(f"Computing route geometry: {cd_name} -> {mdcp_name}")
            ...
            if is_same_location(
                ...
                print(f"Skipping same-location route: {cd_name} -> {mdcp_name}")
                continue
            ...
            try:
                ...
                print(f"Warning: route failed for {cd_name} -> {mdcp_name}: {e}")
                geometry = None
            ...
            if geometry is not None:
                ...
                print(f"Warning: no route returned for {cd_name} -> {mdcp_name}")
...
def build_route_gdf(route_records: list) -> gpd.GeoDataFrame:
    if not route_records:
        return gpd.GeoDataFrame(
            columns=["cd_name", "mdcp_name", "geometry"],
            geometry="geometry",
            crs="EPSG:4326",
        )
...
def get_dynamic_map_bounds(
    ...
    lon_values = (
        ...
        + mobile_distribution_centers["longitude"].tolist()
    )
    lat_values = (
        distribution_centers["latitude"].tolist()
        + mobile_distribution_centers["latitude"].tolist()
    )
...
def plot_routes_on_communes_map(
    ...
    ax.set_title("openrouteservice truck routes from distribution centers to MDCP")
...
def main():
    ...
    output_csv = project_root / "data" / "SOLVER DATA" / "D_CD_MDCP.csv"
    output_image = project_root / "graphs" / "distances" / "D_CD_MDCP_ors_routes.png"
