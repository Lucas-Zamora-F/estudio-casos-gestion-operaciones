...
def call_matrix_api(
    ...
    ds_count = len(dark_stores)

    sources = list(range(cd_count))
    destinations = list(range(cd_count, cd_count + ds_count))
    ...
    distance_matrix = pd.DataFrame(
        ...
        columns=dark_stores["ds_name"].tolist(),
    )
...
def compute_all_route_geometries(
    ...
    for _, cd_row in distribution_centers.iterrows():
        ...
        for _, ds_row in dark_stores.iterrows():
            ds_name = ds_row["ds_name"]
            destination_lat = float(ds_row["latitude"])
            destination_lon = float(ds_row["longitude"])

            print(f"Computing route geometry: {cd_name} -> {ds_name}")
            ...
            if is_same_location(
                ...
                print(f"Skipping same-location route: {cd_name} -> {ds_name}")
                continue
            ...
            try:
                ...
                print(f"Warning: route failed for {cd_name} -> {ds_name}: {e}")
                geometry = None
            ...
            if geometry is not None:
                ...
                print(f"Warning: no route returned for {cd_name} -> {ds_name}")
...
def build_route_gdf(route_records: list) -> gpd.GeoDataFrame:
    if not route_records:
        return gpd.GeoDataFrame(
            columns=["cd_name", "ds_name", "geometry"],
            geometry="geometry",
            crs="EPSG:4326",
        )
...
def get_dynamic_map_bounds(
    ...
    lon_values = (
        ...
        + dark_stores["longitude"].tolist()
    )
    lat_values = (
        distribution_centers["latitude"].tolist()
        + dark_stores["latitude"].tolist()
    )
...
def plot_routes_on_communes_map(
    ...
    ax.scatter(
        dark_stores["longitude"],
        dark_stores["latitude"],
        s=110,
        marker="s",
        color="red",
        edgecolors="black",
    ...
    ax.set_title("openrouteservice truck routes from distribution centers to dark stores")
...
def main():
    ...
    ds_csv = project_root / "data" / "SOLVER DATA" / "DS.csv"
    communes_shp = project_root / "data" / "DPA 2024" / "COMUNAS" / "COMUNAS_v1.shp"

    output_csv = project_root / "data" / "SOLVER DATA" / "D_DC_DS.csv"
    output_image = project_root / "graphs" / "distances" / "D_DC_DS_ors_routes.png"

    distribution_centers = load_points(cd_csv, "cd_name")
    dark_stores = load_points(ds_csv, "ds_name")
